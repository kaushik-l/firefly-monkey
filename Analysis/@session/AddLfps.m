...
function AddLfps(this,prs)    
            ...
            for j=1:prs.linearprobe.channelcount(linearprobe_type)
                ...
                if n == fn
                    if adfreq > prs.fs_lfp
                        [b,a] = butter(2,[prs.lfp_freqmin prs.lfp_freqmax]/(adfreq/2));
                        ad = filtfilt(b,a,ad);
                        N = round(adfreq/prs.fs_lfp); 
                        ad = downsample(ad,N); 
                    end
                    channel_id = j;
    ...
    if ~isempty(utaharray_type) % assume utaharray is recorded using Cereplex
        ...
        file_nev=dir('*.nev'); file_ns1=dir('*.ns1'); file_ns6 = dir('*.ns6'); prs.neur_filetype = 'nev';
        ...
        if length(this.behaviours.trials)==length(events_nev.t_end)
            if isempty(file_ns1), file_ns1 = file_ns6; prs.lfp_filtorder = 2; end % read ns6 if ns1 unavailable 
...
%             if NS1.MetaTags.ChannelCount ~= prs.utaharray.channelcount(utaharray_type), warning('Unexpected channel count in the file \n'); end
            ...
            for j=1:prs.utaharray.channelcount(utaharray_type)
                if mod(j,12)==1, NS1 = openNSx(['/' file_ns1.name],'report','read', 'uV',['c:' num2str(j) ':' num2str(j+11)]); end
                channel_id = NS1.MetaTags.ChannelID(mod(j,12) + 12*(mod(j,12)==0));
                fprintf(['Segmenting LFP :: channel ' num2str(channel_id) '\n']);
                ...
                this.lfps(end).AddTrials(NS1.Data(mod(j,12) + 12*(mod(j,12)==0),:),NS1.MetaTags.SamplingFreq,events_nev,this.behaviours,prs);
            end
        else
