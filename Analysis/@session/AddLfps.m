...
if prs.nev == 1 && prs.plx == 1 %%% delete
    %% load plx
    cd(prs.filepath_neur);
    file_ead_plx=dir('*_ead.plx');
    file_lfp_plx=dir('*_lfp.plx');
    file_ns1=dir('*.ns1'); file_nev=dir('*.nev'); % possible event files
        ...
        if length(this.behaviours.trials)==length(events_plx.t_end)
            ...
            for j=1:prs.maxchannels_plx
                ...
                if n == fn
                    ...
                    this.lfps_plx(j) = lfp(channel_id,electrode_id(ch_id == channel_id),electrode_type,brain_area,coord);% end+1
                    this.lfps_plx(j).AddTrials(ad,adfreq/N,events_plx,this.behaviours,prs); %end
                else
                    fprintf('...... LFP is fragmented. Use a machine with more RAM or contact KL\n');
                end
            end
            %% load nev
            if ~isempty(file_ns1), file_lfp_nev=dir('*.nev');
            ...
            if length(this.behaviours.trials)==length(events_nev.t_end)
                ...
                for j=1:prs.maxchannels_nev
                    ...
                    fprintf(['Segmenting LFP :: channel ' num2str(channel_id) '\n']);
                    this.lfps_nev(j) = lfp(channel_id,electrode_id(ch_id == channel_id)); % end+1
                    this.lfps_nev(j).AddTrials(lfpdata.Data,lfpdata.MetaTags.SamplingFreq,events_nev,this.behaviours,prs); % end
                end
            else
