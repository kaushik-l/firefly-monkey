function eye_movement = AnalyseEyemovement_OFRmodel(eye_fixation,x_fly,y_fly,x_monk,y_monk,zle,yle,zre,yre,t_sac,t_stop,ts,trlerrors,spatialerr,prs)
...
%     t_fix(i) = 0;
    ...
    % reset eye position at trial onset
    if numel(yle{i})>0
        dyle = yle_pred{i}(1) - yle{i}(1); yle{i} = yle{i} + dyle;
        dyre = yre_pred{i}(1) - yre{i}(1); yre{i} = yre{i} + dyre;
        dzle = zle_pred{i}(1) - zle{i}(1); zle{i} = zle{i} + dzle;
        dzre = zre_pred{i}(1) - zre{i}(1); zre{i} = zre{i} + dzre;
    end
    
    % predicte eye position
